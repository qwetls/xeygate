...
- **Provider Governance (ban + creator toggles):** Card-per-provider **My APIs** with collapsible models, live enable/disable toggles for creators, admin Ban/Unban on creator-owned connections, and admin override — a model disabled by an admin cannot be re-enabled by its creator.
...
| `POST` | `/v1/providers/:id/ban` | Ban a creator-owned provider — disables routing + keys (requires admin session) |
| `POST` | `/v1/providers/:id/unban` | Unban a previously banned provider (requires admin session) |
| `POST` | `/v1/providers/mine/:id/models/toggle` | Creator: enable/disable one model — models disabled by admin cannot be re-enabled (Body: `{model_id, action}`) |
