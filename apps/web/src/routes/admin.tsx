...
function AdminLayout() {
            ...
            <main className="flex min-h-svh items-center justify-center bg-background px-4 py-8">
                <Card className="w-full max-w-md">
                    <CardHeader>
                        ...
                        <CardDescription>Could not verify admin session.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Button type="button" variant="outline" onClick={() => void statusQuery.refetch()} className="w-full">Try again</Button>
                    </CardContent>
                </Card>
            </main>
        );
    ...
    if (!statusQuery.data.authenticated) {
        return <AdminAuthForm setupRequired={statusQuery.data.setupRequired} onAuthenticated={() => void statusQuery.refetch()} />;
    }
...
function AdminAuthForm({ setupRequired, onAuthenticated }: { setupRequired: boolean; onAuthenticated: () => void }) {
        ...
        <main className="flex min-h-svh items-center justify-center bg-background px-4 py-8">
            <Card className="w-full max-w-md">
                <CardHeader>
                    ...
                    <CardTitle>{setupRequired ? "Create your admin password" : "Sign in to XEYGATE"}</CardTitle>
                    <CardDescription>
                        {setupRequired
                ...
                <CardContent>
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                        <label className="flex flex-col gap-1.5 text-xs font-medium">
                            Password
                            <Input autoFocus type="password" autoComplete={setupRequired ? "new-password" : "current-password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={setupRequired ? "Choose a password" : "Admin password"} required />
                        </label>
                        {setupRequired && (
                            <label className="flex flex-col gap-1.5 text-xs font-medium">
                                Confirm password
                                <Input type="password" autoComplete="new-password" value={confirmation} onChange={(e) => setConfirmation(e.target.value)} placeholder="Repeat the password" required />
                            </label>
                        )}
                        {setupRequired && (
                            <p className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-[11px] font-normal text-amber-600 dark:text-amber-400">
                                First-come-wins: anyone who opens this page before you finish setup can claim the instance.
                            </p>
                        )}
                        ...
                        <Button type="submit" disabled={isSubmitting} className="mt-1 w-full">
                            {isSubmitting ? "Please wait..." : setupRequired ? "Create password" : "Sign in"}
                        </Button>
                    </form>
                </CardContent>
            </Card>
