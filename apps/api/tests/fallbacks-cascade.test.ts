...
test("ChatLogic logs resolvedModel when srouter/auto meta-model is used", async () => {
    const existingProviders = Array.from(registry.getAllProviders().values());
    for (const prov of existingProviders) {
        registry.unregisterProvider(prov.id);
    }
    ...
    try {
        const autoProvider: AIProvider = {
            ...
            listModels: async () => [
                { id: "auto_test_provider/claude-3-7-sonnet", object: "model" }
            ],
            chatCompletion: async (req) => ({
        ...
        const autoLog = recentLogs.find(
            (l) =>
                l.model === "srouter/auto" &&
                l.resolvedModel === "auto_test_provider/claude-3-7-sonnet"
        );
        ...
        assert.ok(streamLog, "Expected to find streaming log with resolvedModel set");
        assert.equal(streamLog?.resolvedModel, "auto_test_provider/claude-3-7-sonnet");
    } finally {
        registry.unregisterProvider("auto_test_provider");
        for (const prov of existingProviders) {
